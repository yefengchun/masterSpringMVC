...
public class SearchService {
    ...
    {
        //(taking a list of keywords and returning a SearchParameters constructor for each one
        List<SearchParameters> searches = keywords.stream().map(taste -> createSearchParam(searchType,taste))
                .collect(Collectors.toList());

        //to fetch the tweets for each SearchParameters constructor
        List<Tweet> result = searches.stream().map(params -> twitter.searchOperations().search(params))
